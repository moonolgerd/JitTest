...
|------|---------------|----------------|
| `src/WeatherUtilities.cs` | 4 pure static methods, 10+ branch conditions, `StringComparison` | High |
| `src/WeatherForecast.cs` | `TemperatureF` computed property (arithmetic) | Medium |
| `src/WeatherActor.cs` | Duplicate detection, state management, event publishing | High |
| `src/Query.cs` | Actor proxy delegation | Low |
| `src/Mutation.cs` | Actor proxy delegation | Low |
| `src/PublisherHostedService.cs` | Weather data generation, pub/sub publishing | Medium |
